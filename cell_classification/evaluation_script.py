...
if __name__ == "__main__":
    ...
    # check for external validation data and add to test datasets
    datasets = []
    names = []
    if hasattr(model, "external_validation_datasets"):
        datasets.append(model.external_validation_datasets)
        names.append(model.external_validation_names)
    datasets.append(model.test_datasets)
    names.append(model.dataset_names)
    # calculate metrics
    for dset, name in zip(datasets, names):
        params["eval_dir"] = os.path.join(*os.path.split(params["model_path"])[:-1], "eval", name)
        os.makedirs(params["eval_dir"], exist_ok=True)
        pred_list = model.predict_dataset(dset, False)
